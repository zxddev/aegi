...
### Requirement: GraphRAG MUST extract entities/events/relations from intelligence text
GraphRAG MUST 从情报文本中抽取实体、事件和关系，并写入 Neo4j。
...
LLM 不可用时系统 MUST 返回硬错误，不降级为规则引擎。
...
### Requirement: System MUST deduplicate entities
系统 MUST 对实体进行去重，避免重复节点。
