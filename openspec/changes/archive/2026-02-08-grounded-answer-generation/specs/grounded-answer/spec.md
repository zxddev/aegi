...
### Requirement: 证据格式化 MUST 在回答前完成

chat 回答前，系统 MUST 将检索到的 SourceClaim 格式化为编号证据上下文字符串。
...
### Requirement: LLM Grounded 回答 MUST 带内联引用

系统 MUST 用 LLM 基于编号证据生成带内联引用的回答。
...
### Requirement: Hallucination gate MUST 保留

现有 hallucination gate 逻辑 MUST 保持不变。
